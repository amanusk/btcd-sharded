...
// SqlMaybeAcceptBlock potentially accepts a block into the block chain and, if
// accepted, returns whether or not it is on the main chain.  It performs
// several validation checks which depend on its position within the block chain
// before adding it.  The block is expected to have already gone through
// ProcessBlock before calling this function with it.
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) SqlMaybeAcceptBlock(block *btcutil.Block, flags BehaviorFlags) (bool, error) {
	// The height of this block is one more than the referenced previous
	// block.
	prevHash := &block.MsgBlock().Header.PrevBlock
	prevNode := b.index.LookupNode(prevHash)
	...
	// selection according to the chain with the most proof of work.  This
	// also handles validation of the transaction scripts.
	isMainChain, err := b.SqlConnectBestChain(newNode, block, flags)
	if err != nil {
		return false, err
	}
	//
...
func (b *BlockChain) CoordMaybeAcceptBlock(header *wire.BlockHeader, flags BehaviorFlags) (bool, error) {
	...
	// Note: this is replaced by each shard
	// Connect the passed block to the chain while respecting proper chain
	// selection according to the chain with the most proof of work.  This
	// also handles validation of the transaction scripts.
	//isMainChain, err := b.SqlConnectBestChain(newNode, block, flags)
