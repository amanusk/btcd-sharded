...
//
// This function MUST be called with the chain state lock held (for writes).
func sqlConnectBlock(db database.DB, block *btcutil.Block, view UtxoView, stxos []spentTxOut) error {
	...
	// TODO : improve this sainity check to differ between coinbaes/noncoinbase
	// Sanity check the correct number of stxos are provided.
	//if len(stxos) != countSpentOutputs(block) {
...
// CoordConnectBestChain handles connecting the passed block to the chain while
// respecting proper chain selection according to the chain with the most
// proof of work.  In the typical case, the new block simply extends the main
// chain.  However, it may also be extending (or creating) a side chain (fork)
// which may or may not end up becoming the main chain depending on which fork
...
func (shard *Shard) ShardConnectBestChain(node *BlockNode, block *btcutil.Block, flags BehaviorFlags) (bool, error) {
	...
	if true {
		...
		logging.Println("Connecting block", block.Hash(), " height", node.height)
		...
		if err != nil {
			if _, ok := err.(RuleError); ok {
				logging.Println("Block validation failed ", err)
				// TODO: reenalbe
				// b.index.SetStatusFlags(node, statusValidateFailed)
			}
			return false, err
...
func New(config *Config) (*BlockChain, error) {
	...
	logging.Printf("Chain state (height %d, hash %v, totaltx %d, work %v)",
