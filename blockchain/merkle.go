...
// BuildMerkleRoot creates a merkle tree, but out of the ready transaction hashes
func BuildMerkleRoot(hashes []*chainhash.Hash) []*chainhash.Hash {
	...
	for level := uint(0); level < uint(exponent); level++ {
		...
		for n := 0; n < workers; n++ {
			readSlice := merkles[readStart+(items*n) : readStart+(items*(n+1))]
			writeSlice := merkles[writeStart+((items/2)*n) : writeStart+((items/2)*(n+1))]
	...
	// fmt.Println("New Merkle root took", endTime)
