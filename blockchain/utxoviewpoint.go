...
type UtxoEntry struct {
	...
	// NOTE: The memebers were made public to allow encoding and decoding
...
type UtxoView interface {
	...
	AddExplicitTxOut(outpoint wire.OutPoint, utxoEntry *UtxoEntry)
	...
	SQLFetchUtxosMain(db *SQLBlockDB, outpoints map[wire.OutPoint]struct{}) error
...
var _ UtxoView = (*UtxoViewpoint)(nil)
...
// AddExplicitTxOut adds an output to the view that chould have been received in another way
func (view *UtxoViewpoint) AddExplicitTxOut(outpoint wire.OutPoint, utxoEntry *UtxoEntry) {
...
func (view *UtxoViewpoint) DisconnectTransactions(db database.DB, block btcutil.Block, stxos []spentTxOut) error {
	...
	for txIdx := len(transactions) - 1; txIdx > -1; txIdx-- {
		...
		for txInIdx := len(tx.MsgTx().TxIn) - 1; txInIdx > -1; txInIdx-- {
			...
			if stxo.height == 0 {
				...
				stxo.height = utxo.GetBlockHeight()
	...
	view.SetBestHash(&block.Header().PrevBlock)
	return nil
}
...
func (view *UtxoViewpoint) SQLFetchUtxosMain(db *SQLBlockDB, outpoints map[wire.OutPoint]struct{}) error {
	// Nothing to do if there are no requested outputs.
	if len(outpoints) == 0 {
		return nil
	}
