...
func (shard *Shard) CheckShardedTransactionInputs(tx *btcutil.Tx, txHeight int32, utxoView UtxoView, chainParams *chaincfg.Params, localTxFilter *bloom.Filter, localMissingTxList []*wire.OutPoint) (int64, error) {
...
func (shard *Shard) ShardCheckConnectBlock(node *BlockNode, block btcutil.Block, view UtxoView, stxos *[]spentTxOut) error {
	...
	// for txOut, utxo := range view.Entries() {
	// 	logging.Println(txOut, " UTXO", utxo)
	// }
	...
	// bounds.
	var totalFees int64
	// TODO: pass chain params to shards
	...
	if runScripts {
		...
		//	b.hashCache)
		err := checkBlockScripts(block, view, scriptFlags, nil,
			nil)
