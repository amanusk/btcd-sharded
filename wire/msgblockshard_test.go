...
func TestBlockShard(t *testing.T) {
	pver := ProtocolVersion

	// Block 1 header.
	prevHash := &blockShardOne.Header.PrevBlock
	merkleHash := &blockShardOne.Header.MerkleRoot
	bits := blockShardOne.Header.Bits
	nonce := blockShardOne.Header.Nonce
	bh := NewBlockHeader(1, prevHash, merkleHash, bits, nonce)

	// Ensure the command is expected value.
	wantCmd := "block"
	msg := NewMsgBlockShard(bh)
	...
	tx := blockShardOne.Transactions[0].Copy()
	msg.AddTransaction(tx)
	if !reflect.DeepEqual(msg.Transactions, blockShardOne.Transactions) {
		t.Errorf("AddTransaction: wrong transactions - got %v, want %v",
			spew.Sdump(msg.Transactions),
			spew.Sdump(blockShardOne.Transactions))
	}
...
func TestBlockShardTxHashes(t *testing.T) {
	...
	hashes, err := blockShardOne.TxHashes()
	if err != nil {
...
func TestBlockShardHash(t *testing.T) {
	// Block 1 hash.
	...
	blockHash := blockShardOne.BlockHash()
...
func TestBlockShardWire(t *testing.T) {
	tests := []struct {
		in     *MsgBlockShard  // Message to encode
		out    *MsgBlockShard  // Expected decoded message
...
//// of MsgBlockShard to confirm error paths work correctly.
func TestBlockShardWireErrors(t *testing.T) {
	...
	tests := []struct {
		in       *MsgBlockShard  // Value to encode
		buf      []byte          // Wire encoding
		pver     uint32          // Protocol version for wire encoding
		enc      MessageEncoding // Message encoding format
		...
		{&blockShardOne, blockShardOneBytes, pver, BaseEncoding, 0, io.ErrShortWrite, io.EOF},
		// Force error in prev block hash.
		{&blockShardOne, blockShardOneBytes, pver, BaseEncoding, 4, io.ErrShortWrite, io.EOF},
		// Force error in merkle root.
		{&blockShardOne, blockShardOneBytes, pver, BaseEncoding, 36, io.ErrShortWrite, io.EOF},
		// Force error in timestamp.
		{&blockShardOne, blockShardOneBytes, pver, BaseEncoding, 68, io.ErrShortWrite, io.EOF},
		// Force error in difficulty bits.
		{&blockShardOne, blockShardOneBytes, pver, BaseEncoding, 72, io.ErrShortWrite, io.EOF},
		// Force error in header nonce.
		{&blockShardOne, blockShardOneBytes, pver, BaseEncoding, 76, io.ErrShortWrite, io.EOF},
		// Force error in transaction count.
		{&blockShardOne, blockShardOneBytes, pver, BaseEncoding, 80, io.ErrShortWrite, io.EOF},
		// Force error in transactions.
		{&blockShardOne, blockShardOneBytes, pver, BaseEncoding, 81, io.ErrShortWrite, io.EOF},
	}
...
//// TestBlockSerialize tests MsgBlockShard serialize and deserialize.
func TestBlockShardSerialize(t *testing.T) {
	tests := []struct {
		in     *MsgBlockShard // Message to encode
		out    *MsgBlockShard // Expected decoded message
...
//// decode of MsgBlockShard to confirm error paths work correctly.
func TestBlockShardSerializeErrors(t *testing.T) {
	tests := []struct {
		in       *MsgBlockShard // Value to encode
		buf      []byte         // Serialized data
		max      int            // Max size of fixed buffer to induce errors
		...
		// Force error in version.
		{&blockShardOne, blockShardOneBytes, 0, io.ErrShortWrite, io.EOF},
		// Force error in prev block hash.
		{&blockShardOne, blockShardOneBytes, 4, io.ErrShortWrite, io.EOF},
		// Force error in merkle root.
		{&blockShardOne, blockShardOneBytes, 36, io.ErrShortWrite, io.EOF},
		// Force error in timestamp.
		{&blockShardOne, blockShardOneBytes, 68, io.ErrShortWrite, io.EOF},
		// Force error in difficulty bits.
		{&blockShardOne, blockShardOneBytes, 72, io.ErrShortWrite, io.EOF},
		// Force error in header nonce.
		{&blockShardOne, blockShardOneBytes, 76, io.ErrShortWrite, io.EOF},
		// Force error in transaction count.
		{&blockShardOne, blockShardOneBytes, 80, io.ErrShortWrite, io.EOF},
		// Force error in transactions.
		{&blockShardOne, blockShardOneBytes, 81, io.ErrShortWrite, io.EOF},
	}
...
// vector.
func TestBlockShardOverflowErrors(t *testing.T) {
...
func TestBlockShardSerializeSize(t *testing.T) {
	// Block with no transactions.
	noTxBlock := NewMsgBlockShard(&blockShardOne.Header)

	tests := []struct {
		in   *MsgBlockShard // Block to encode
		...
		{&blockShardOne, len(blockShardOneBytes)},
	}
...
var blockShardOne = MsgBlockShard{
	Header: BlockHeader{
		Version: 1,
